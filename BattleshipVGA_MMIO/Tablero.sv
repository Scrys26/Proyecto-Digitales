...
module Tablero (
    ...
    // ---- Taps para superposicion externa  ----
    output logic       clk_25M_tap,
    output logic       in_vis_tap,
    output logic       in_board_tap,
    output logic [9:0] x_rel_tap,
    output logic [9:0] y_rel_tap,
    output logic       on_grid_tap,
    output logic       text_on_tap,
    // NUEVO: coordenadas absolutas visibles
    output logic [9:0] x_vis_tap,
    output logic [9:0] y_vis_tap
);
    // -------------------- Modulos --------------------
    logic        clk_25M;
    ...
    // -------------------- Tablero 10x10 centrado (320x320) --------------------
    ...
    logic       slot_1    = (x_off_left >= (TEXT_W + 1));      // 2do dígito
    ...
    function automatic logic [7:0] font8x8 (input logic [7:0] ch, input logic [2:0] r);
        // (… tu tabla de fuente exacta aquí, sin cambios …)
        // He omitido por brevedad la tabla que ya pegaste; copia la misma.
        case (ch)
     // A..J
        "A": case(r) 
    ...
    always_comb begin
        if (!in_vis) begin
            base_r = 4'h0; base_g = 4'h0; base_b = 4'h0;
        end else begin
            // fondo visible
            base_r = 4'hF; base_g = 4'hF; base_b = 4'hF;

            if (in_board) begin
                base_r = 4'h9; base_g = 4'hD; base_b = 4'hF;
            end

            if (text_on || on_grid) begin
                base_r = 4'h0; base_g = 4'h0; base_b = 4'h0;
            end
        end
    end
    ...
    // Taps
    assign clk_25M_tap  = clk_25M;
    assign in_vis_tap   = in_vis;
    assign in_board_tap = in_board;
    assign x_rel_tap    = x_rel;
    assign y_rel_tap    = y_rel;
    assign on_grid_tap  = on_grid;
    assign text_on_tap  = text_on;
    assign x_vis_tap    = x_vis;   // NUEVO
    assign y_vis_tap    = y_vis;   // NUEVO

    // Salida base del tablero
    assign vgaRed   = base_r;
    assign vgaGreen = base_g;
    assign vgaBlue  = base_b;
endmodule
