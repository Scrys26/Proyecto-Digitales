...
# =========================
    ...
    # Control overlay splash (bit0 = enable)
    .equ VGA_SPLASH_CTRL, 0x0001009C
...
############################################################
# PANTALLA DE BIENVENIDA (splash overlay)
#   - Enciende el logo en VGA.
#   - Espera a que el jugador pulse CONFIRM en el mando.
#   - Apaga el splash y continúa con la lógica original.
#   Sólo usa registros temporales t0..t3.
############################################################
    # Encender splash (bit0 = 1)
    li  t0, VGA_SPLASH_CTRL
    li  t1, 1
    sw  t1, 0(t0)

splash_wait_confirm:
    li  t0, MANDO_STA
    lw  t1, 0(t0)          # leer botones actuales
    li  t2, BIT_CONFIRM    # máscara 0x1
    and t3, t1, t2
    beqz t3, splash_wait_confirm  # mientras CONFIRM=0, seguir esperando
...
############################################################
# Inicialización de estado (lógica original intacta)
############################################################
